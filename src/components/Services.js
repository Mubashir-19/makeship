                ...
                <div className="grid grid-cols-1 md:grid-cols-3 md:auto-rows-[minmax(180px,auto)] gap-4 lg:gap-6">
                    ...
                    <div className="md:col-span-2 md:row-span-2">
                        <Reveal delay={100} className="h-full">
                            <div className={`h-full min-h-[300px] bg-gradient-to-br ${isDarkMode ? 'from-amber-900/40 to-slate-900' : 'from-amber-100 to-white'} border border-[var(--border)] rounded-3xl p-8 relative overflow-hidden hover:border-amber-500/50 hover:scale-[1.01] transition-all duration-300 group shadow-lg`}>
                                <div className="absolute top-0 right-0 p-8 opacity-20 group-hover:opacity-40 transition-opacity">
                                    <MessageSquareCode className={`w-32 h-32 text-[var(--accent-amber)]`} />
                                </div>
                                <div className="relative z-10 flex flex-col justify-between h-full space-y-4">
                                    <div>
                                        <div className="w-12 h-12 rounded-xl bg-amber-500/20 flex items-center justify-center mb-4 text-[var(--accent-amber)]">
                                    ...
                                    <div className="flex flex-wrap gap-2 pt-4">
                                        <span className="text-xs font-mono px-2 py-1 rounded bg-amber-500/10 border border-amber-500/20 text-[var(--accent-amber)]">LLM Integration</span>
                                        <span className="text-xs font-mono px-2 py-1 rounded bg-amber-500/10 border border-amber-500/20 text-[var(--accent-amber)]">RAG Pipelines</span>
                                    </div>
                                </div>
                            </div>
                        </Reveal>
                        ...
                        <Reveal delay={200} className="h-full">
                            <div className="h-full min-h-[200px] bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-6 hover:bg-opacity-80 hover:scale-[1.02] transition-all duration-300 group flex flex-col justify-between shadow-sm hover:shadow-md">
                                <div className="flex items-center justify-between mb-4">
                                    <ShieldCheck className="w-8 h-8 text-[var(--accent-amber)]" />
                                    <ArrowRight className="w-5 h-5 text-[var(--text-muted)] group-hover:text-[var(--accent-amber)] transition-colors -rotate-45 group-hover:rotate-0" />
                                </div>
                                <div>
                        ...
                        <Reveal delay={300} className="h-full">
                            <div className="h-full min-h-[300px] bg-[var(--bg-card)] border border-[var(--border)] rounded-3xl p-6 flex flex-col justify-end hover:bg-opacity-80 hover:scale-[1.02] transition-all duration-300 relative overflow-hidden shadow-sm hover:shadow-md">
                                ...
                                <div className="relative z-10">
                                    <Globe className="w-10 h-10 text-[var(--accent-amber)] mb-4" />
                    ...
                    <div className="md:col-span-1 md:row-span-1">
                        <Reveal delay={500} className="h-full">
                            <div className="h-full min-h-[150px] bg-gradient-to-r from-[var(--bg-card-solid)] to-[var(--bg-input)] border border-[var(--border)] rounded-3xl p-6 flex items-center justify-center text-center cursor-pointer hover:border-[var(--accent-amber)] hover:scale-[1.02] transition-all duration-300 shadow-sm" onClick={() => scrollToSection(3)}>
                                <div className="group">
                                    <h3 className="text-lg font-bold text-[var(--text-main)] group-hover:text-[var(--accent-amber)] transition-colors">View More</h3>
